...
def main():
            ...
            try:
                with open(r_path, 'r', encoding='utf-8') as rf:
                    ...
                    for m in matches:
                        ...
                        ctrl = ctrl_match.group(1) if ctrl_match else "Included in route"
                        
                        # Role extraction
                        roles_match = re.search(r'authorize(?:Roles)?\((.*?)\)', handlers)
                        roles = "Default/Token Required"
                        if roles_match:
                            r_str = roles_match.group(1).replace("'", "").replace('"', "").strip()
                        ...
                        out += f"| `{ep}` | `{method}` | `{ctrl}` | `{service}` | `{roles}` | `{payload}` | `{response}` |\n"
            except:
                pass
    out += "\n"
                ...
                with open(m_path, 'r', encoding='utf-8') as sql_f:
                    ...
                    for tm in tbl_matches:
                        ...
                        for bl in t_body.split('\n'):
                            ...
                            parts = bl.split(' ')
                            if len(parts) >= 2 and not parts[0].startswith('CHECK'):
    ...
    out += "## 4️⃣ Middleware\n\n"
    mw_configs = {
        'auth.middleware.js': ['authenticateToken', 'authorizeRoles', 'verifyFirebaseToken'],
        'role.middleware.js': ['checkRoleLevel', 'roleHierarchyUtils'],
        'upload.middleware.js': ['multer', 'uploadSchema'],
        'rate_limit.middleware.js': ['rateLimit', 'authLimiter', 'searchLimiter'],
    ...
    known_events = ['LEAVE_APPROVED', 'LEAVE_STATUS_CHANGED', 'OD_APPROVED', 'RESULT_UPLOADED', 'FEED_POST_CREATED', 'ATTENDANCE_UPDATED', 'NOTIFICATION_CREATED']
    events_found = set()
